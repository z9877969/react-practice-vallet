...
export const searchCategoryName = (category, item, options) => {
  ...
  return optionsArr
    .find(([key]) => key.includes(category))[1]
    .options.find((el) => el.value === item).title;
};
...
export const getDataByCategory = (category, incomeData, spendData) =>
  category === "outlay" ? spendData : category === "income" ? incomeData : [];
